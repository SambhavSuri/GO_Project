...
def handle_chat(data):
    try:
        ...
        # Emit thinking state - RAG processing started
        logger.info("🤔 Emitting thinking_started event to all clients")
        emit('thinking_started', {
            'type': 'thinking_started',
            'message': 'Processing your query...',
            'vrmData': VRMData(
                animations={"current": "thinking"},
                expressions={"current": "thinking"}
            ).to_dict()
        }, broadcast=True)
        logger.info("✅ thinking_started event emitted")
...
def handle_tts_finished():
    logger.info("TTS playback finished - stopping speaking animation")
    # Emit TTS finished event to stop speaking animation and return to idle
    logger.info("🔇 Emitting tts_ended event to all clients")
    emit('tts_ended', {
        'type': 'tts_ended',
        'vrmData': VRMData(
            animations={"current": "idle"},
            expressions={"current": "neutral"}
        ).to_dict(),
        'message': 'Speech finished'
    }, broadcast=True)
    logger.info("✅ tts_ended event emitted")
...
def generate_talking_vrm_response(text):
    """Generate VRM data specifically for talking state with face expressions"""
    # This uses the talking.fbx animation with appropriate expressions
    text_lower = text.lower()
    ...
    return VRMData(
        animations={"current": "talking", "type": "speaking"},
        expressions={"current": expression, "speaking": True}
    )
