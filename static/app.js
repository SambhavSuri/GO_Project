...
// Check if animation functions are ready
function areAnimationFunctionsReady() {
    return !!(window.switchToThinking && window.startSynchronizedSpeaking && window.stopSpeaking);
}

// Create fallback animation functions
function createFallbackAnimationFunctions() {
    if (!window.switchToThinking) {
        window.switchToThinking = () => console.warn('🔄 switchToThinking fallback called');
    }
    if (!window.startSynchronizedSpeaking) {
        window.startSynchronizedSpeaking = () => console.warn('🔄 startSynchronizedSpeaking fallback called');
    }
    if (!window.stopSpeaking) {
        window.stopSpeaking = () => console.warn('🔄 stopSpeaking fallback called');
    }
    console.log('🔄 Created fallback animation functions');
}

// Wait for animation functions to be ready
function waitForAnimationFunctions(callback, maxRetries = 10) {
    if (areAnimationFunctionsReady()) {
        console.log('✅ Animation functions are ready');
        callback();
    } else if (maxRetries > 0) {
        console.log('⏳ Waiting for animation functions to load...');
        setTimeout(() => {
            waitForAnimationFunctions(callback, maxRetries - 1);
        }, 200);
    } else {
        console.warn('⚠️ Animation functions not ready after waiting, creating fallbacks');
        createFallbackAnimationFunctions();
        callback();
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Wait for animation functions before initializing socket
    waitForAnimationFunctions(() => {
        initializeSocket();
        console.log('🎉 Full system initialized with animation support');
...
function initializeSocket() {
    ...
    socket.on('thinking_started', (data) => {
        console.log('AI started thinking:', data);
        addSystemMessage('🤔 AI is thinking...');
        
        // Switch to thinking animation with retry mechanism
        const tryThinking = () => {
            console.log('window.switchToThinking available:', typeof window.switchToThinking);
            if (window.switchToThinking) {
        ...
        if (!tryThinking()) {
            console.warn('⏳ Animation functions not ready, retrying...');
            // Retry after a short delay
            setTimeout(() => {
                if (!tryThinking()) {
                    console.warn('❌ window.switchToThinking still not available after retry');
                }
            }, 500);
        }
        updateVRMState(data.vrmData);
    ...
    socket.on('rag_completed', (data) => {
        console.log('RAG processing completed:', data);
        addSystemMessage('🧠 Processing completed, generating speech...');
    });
    
    socket.on('tts_started', (data) => {
        console.log('TTS started, switching to talking animation:', data);
        addSystemMessage('🎤 AI started speaking...');
        
        // Use synchronized speaking with retry mechanism
        const tryTalking = () => {
            console.log('Available animation functions:', {
                startSynchronizedSpeaking: typeof window.startSynchronizedSpeaking,
                startSpeaking: typeof window.startSpeaking,
                switchToTalking: typeof window.switchToTalking
            });
        ...
        if (!tryTalking()) {
            console.warn('⏳ Speaking animation functions not ready, retrying...');
            // Retry after a short delay
            setTimeout(() => {
                if (!tryTalking()) {
                    console.warn('❌ No speaking animation functions available after retry');
                }
            }, 500);
        }
        updateVRMState(data.vrmData);
    ...
    socket.on('tts_ended', (data) => {
        console.log('TTS ended, stopping speaking animation:', data);
        
        // Stop speaking animation with retry mechanism
        const tryStopSpeaking = () => {
            console.log('window.stopSpeaking available:', typeof window.stopSpeaking);
            if (window.stopSpeaking) {
        ...
        if (!tryStopSpeaking()) {
            console.warn('⏳ Stop speaking function not ready, retrying...');
            // Retry after a short delay
            setTimeout(() => {
                if (!tryStopSpeaking()) {
                    console.warn('❌ window.stopSpeaking still not available after retry');
                }
            }, 500);
        }
        updateVRMState(data.vrmData);
...
function initializeEventListeners() {
    ...
    document.getElementById('test-thinking').addEventListener('click', () => {
        console.log('🧪 Testing thinking animation...');
        if (window.switchToThinking) {
            window.switchToThinking();
        } else {
            console.error('switchToThinking not available');
        }
    });
    
    document.getElementById('test-talking').addEventListener('click', () => {
        console.log('🧪 Testing talking animation...');
        if (window.startSynchronizedSpeaking) {
            window.startSynchronizedSpeaking();
        } else if (window.switchToTalking) {
            window.switchToTalking();
        } else {
            console.error('Talking animations not available');
        }
    });
    ...
    document.getElementById('test-idle').addEventListener('click', () => {
        ...
        if (window.stopSpeaking) {
            window.stopSpeaking();
        } else if (window.switchToIdle) {
            window.switchToIdle();
        } else {
            console.error('Idle animations not available');
        }
    });
    
    // Add debug button for full system check
    setTimeout(() => {
        if (window.debugAnimationSystem) {
            console.log('🔧 Running initial animation system debug...');
            window.debugAnimationSystem();
        } else {
            console.log('🔧 Debug function not available, checking manually:');
            console.log('Animation functions available:', {
                switchToThinking: typeof window.switchToThinking,
                startSynchronizedSpeaking: typeof window.startSynchronizedSpeaking,
                stopSpeaking: typeof window.stopSpeaking
            });
        }
    }, 3000); // Wait 3 seconds for VRM to load
...
function playAudio(audioUrl) {
    ...
    conversationAudio.onended = () => {
        ...
        // Notify server that TTS has finished
        socket.emit('tts_finished');
        console.log('TTS audio finished, notified server');
    };
